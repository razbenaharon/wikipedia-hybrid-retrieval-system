...
BM25_B_GRID = (0.5, 0.75, 0.9)


class CoreScores:
    def __init__(self, semantic_norm: np.ndarray, title_scores: np.ndarray) -> None:
...
def _title_score_matrix(queries: List[str], artifacts) -> np.ndarray:
    ...
    for row_idx, query in enumerate(queries):
        ...
        for doc_idx, title_terms in enumerate(artifacts.title_token_sets):
            if title_terms:
                title_scores[row_idx, doc_idx] = len(query_terms & title_terms) / denom
    return title_scores
...
def run_grid_search(
    ...
    *,
    ...
    for sem_units, lex_units, cov_units, title_units in iter_weight_units(GRID_UNITS):
        weights = Weights(
...
def iter_weight_units(total_units: int) -> Iterable[Tuple[int, int, int, int]]:
    for semantic in range(total_units + 1):
        for lexical in range(total_units - semantic + 1):
            for coverage in range(total_units - semantic - lexical + 1):
                title = total_units - semantic - lexical - coverage
                yield semantic, lexical, coverage, title
